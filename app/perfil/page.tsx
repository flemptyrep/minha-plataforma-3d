...
export default function Perfil() {
  ...
  const handleAtualizarPerfil = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Fazer upload da nova foto (se o utilizador selecionou alguma)
      if (fileInputRef.current?.files && fileInputRef.current.files.length > 0) {
        const file = fileInputRef.current.files[0];
        const fileExt = file.name.split('.').pop();
        const fileName = `avatar_${Date.now()}.${fileExt}`;

        // Usamos o bucket 'fotos' já existente
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6 md:p-16 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          {/* LADO ESQUERDO: DEFINIÇÕES DA CONTA */}
            ...
            <form onSubmit={handleAtualizarPerfil} className="bg-gray-900 border border-gray-800 p-8 rounded-[2rem] shadow-xl space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input type="password" placeholder="******" value={novaSenha} onChange={e => setNovaSenha(e.target.value)}
                  className="w-full p-4 bg-gray-950 border border-gray-800 rounded-2xl focus:border-blue-500 outline-none transition-colors placeholder-gray-700" />
              </div>

              <button type="submit" disabled={atualizando}
                className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl hover:bg-blue-500 transition shadow-lg shadow-blue-600/20 uppercase tracking-widest text-xs mt-4">
                {atualizando ? t.atualizando : t.guardarAlteracoes}
              </button>
            </form>
          </div>

          {/* LADO DIREITO: MEUS PROJETOS */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {projetos.map((p) => (
                  <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-[2rem] overflow-hidden relative shadow-xl hover:border-blue-500/30 transition-colors">
                    ...
                    <div className={`absolute top-4 right-4 z-10 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-xl ${
                      p.status === 'aprovado' ? 'bg-emerald-500 text-black' : p.status === 'recusado' ? 'bg-red-500 text-white' : 'bg-amber-500 text-black'
                    }`}>
                      {p.status === 'aprovado' ? t.aprovado : p.status === 'recusado' ? t.recusado : t.pendente}
                    </div>
